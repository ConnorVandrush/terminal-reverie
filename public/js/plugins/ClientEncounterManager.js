class ClientEncounterManager {
  ...
  startEncounter({ troopData, enemyData, battleback }) {
    ...
    this.clientPartyManager.partyData.members.forEach((member, index) => {
      ...
      window.clientGlobalManager.clientPlayerManager.applyBattlerSprite(
        member,
        actorId,
      );
    ...
    window.clientGlobalManager.dispatchToReact({
      type: "encounter/setCurrentTarget",
      payload: null,
    });
    window.clientGlobalManager.dispatchToReact({
      type: "encounter/setEnemyInfo",
      payload: enemyData.map((enemy) => ({
        name: enemy.name,
        maxHp: enemy.maxHp,
    ...
    window.clientGlobalManager.dispatchToReact({
      type: "encounter/setAllyInfo",
      payload: this.clientPartyManager.partyData.members.map((member) => ({
        name: member.name,
        maxHp: member.maxHp,
  ...
  async processAllyAttackResult(
    ...
    window.clientGlobalManager.dispatchToReact({
      type: "encounter/appendEncounterMessage",
      payload: encounterMessage,
    });
    const index =
      window.clientGlobalManager.clientPartyManager.findPartyIndex(playerId);
  ...
  async processEnemyAttackResult(
    enemyIndex,
    targetIndex,
    damage,
    encounterMessage,
    ...
    this.socket.on(
      ...
      async (
        ...
        for (const result of allyTurnResults) {
          ...
          switch (actionType) {
            case "attack":
              await this.processAllyAttackResult(
                playerId,
                targetIndex,
                damage,
                encounterMessage,
        ...
        for (const result of enemyTurnResults) {
          ...
          switch (actionType) {
            case "attack":
              await this.processEnemyAttackResult(
                enemyIndex,
                targetIndex,
                damage,
                encounterMessage,
        ...
        if (
          deadAllies.length === this.clientPartyManager.partyData.members.length
        ) {
          ...
          const characterData =
            window.clientGlobalManager.clientPartyManager.partyData.members.find(
              (member) =>
                member.playerId ===
          ...
          window.clientGlobalManager.dispatchToReact({
            type: "encounter/appendEncounterMessage",
            payload: rewards.rewardMessage,
          });
          await this.wait(3000);
          const response = await this.socket.emitWithAck(
            "clientRequestTransferFromEncounter",
          );
          if (response.success) {
            window.clientGlobalManager.dispatchToReact({
              type: "encounter/clearEncounterMessages",
            });
            window.clientGlobalManager.dispatchToReact({
              type: "centerPanel/setCenterPanel",
              payload: null,
            ...
            window.clientGlobalManager.clientMapManager.transferToMap(
              response.mapData,
              response.tileset,
              response.x,
              response.y,
...
window.clientGlobalManager.clientEncounterManager =
  new ClientEncounterManager();
