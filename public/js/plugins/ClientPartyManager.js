class ClientPartyManager {
  ...
  findPartyIndex(playerId) {
    return window.clientGlobalManager.clientPartyManager.partyData.members.findIndex(
  ...
  startListeners() {
    ...
    socket.on("serverUpdatePartyData", (partyData) => {
      ...
      window.clientGlobalManager.dispatchToReact({
        ...
        payload: frozenReactPartyData,
      });
      if (
        partyData !== null &&
        partyData.members[0].playerId ===
          this.clientPlayerManager.characterData.playerId
      ) {
        ...
        this.clientMapManager.moveToLocation(
          $gamePlayer,
          partyData.members[0].location,
        );
        ...
        window.clientGlobalManager.clientPartyManager.partyData = {
          members: characterData ? [characterData] : [],
        };
        window.clientGlobalManager.dispatchToReact({
          type: "partyWindow/setPartyData",
          payload: { members: characterData ? [characterData] : [] },
        });
      }
    });
  }
