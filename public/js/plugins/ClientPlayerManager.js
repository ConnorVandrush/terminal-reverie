class ClientPlayerManager {
  ...
  async login(characterData, { mapData, tileset }, playersOnMap) {
    ...
    window.clientGlobalManager.clientPlayerManager.socket = io(
      window.clientGlobalManager.SERVER_CONFIG.IO_URL,
      {
        auth: { JWT },
        reconnection: true,
    ...
    const appearance = await this.spriteColorer.recolorSpritesheet(
      ...
      characterData.appearance.template,
      characterData.appearance.colors,
    );
    const bitmap = ImageManager.loadBitmapFromUrl(appearance);
    $gamePlayer._customBitmap = bitmap;

    window.clientGlobalManager.clientPlayerManager.customBitmaps.set(
      String("$" + characterData.playerId),
      bitmap,
    );
    $gameActors
      .actor(1)
      .setCharacterImage(String("$" + characterData.playerId), 0);
      ...
      pages: [
        {
          ...
          image: {
            characterName: String("$" + characterData.playerId),
            characterIndex: 0,
  ...
  updatePlayerCharacterData(newCharacterData) {
    const index =
      window.clientGlobalManager.clientPartyManager.partyData.members.findIndex(
        (member) => member.playerId === newCharacterData.playerId,
      );
    ...
    window.clientGlobalManager.dispatchToReact({
      ...
      payload: newCharacterData.inventory,
    });
  }
