class ClientMapManager {
  ...
  async requestMove(direction, oldLoc) {
    try {
      ...
      if (!response?.success) {
        ...
        const loc =
          window.clientGlobalManager.clientPlayerManager.characterData.location;
        loc.x = response.newLocation.x;
        loc.y = response.newLocation.y;

        if (response.encounter != null) {
          window.clientGlobalManager.clientEncounterManager.startEncounter(
            response.encounter,
          );
        }
      }
  ...
  moveToLocation(player, newLoc) {
    while (player.x !== newLoc.x || player.y !== newLoc.y) {
      ...
      this.waitForMovementEnd(player);
      if (
        !window.clientGlobalManager.clientPartyManager.isPartyLeader &&
        !window.clientGlobalManager.clientPartyManager.isPartyFollower
      ) {
  ...
  transferToMap(mapData, tileset, x, y, d, playersOnMap) {
    ...
    window.clientGlobalManager.clientPlayerManager.pendingRemotePlayers =
      new Map(playersOnMap);
  ...
  startListeners() {
    ...
    socket.on("serverPlayerMoved", ({ playerId, newLocation }) => {
      ...
      const eventId =
        window.clientGlobalManager.clientPlayerManager.playersOnMap.get(
          playerId,
        )?.eventId;
