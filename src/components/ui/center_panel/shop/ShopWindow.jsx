...
export default function ShopWindow() {
  ...
  const selectedShopItem = useSelector(
    (state) => state.shopWindow.selectedShopItem,
  );
  const characterData = useSelector((state) =>
    state.partyWindow.partyData.members.find(
      (member) =>
        member.playerId ===
  ...
  return (
    <div className={styles.shopWindow}>
      <div className={styles.shopHeader}>
        ...
        <div className={styles.characterGold}>Gold: {characterData.gold}</div>
      </div>
      ...
      <div className={styles.buttonContainer}>
        <div className={styles.buyButtonContainer}>
          <button
            ...
            onClick={() =>
              dispatch(clientBuyItem({ itemId: selectedShopItem, quantity: 1 }))
