...
export default function PartyInvites() {
  ...
  const partyInvites = useSelector(
    (state) => state.partyWindow.partyInvites || [],
  );
  const errorMessage = useSelector(
    (state) => state.partyWindow.errorMessage || null,
  );
  const successMessage = useSelector(
    (state) => state.partyWindow.successMessage || null,
  );
      ...
      <div className={styles.invitesList}>
        ...
        {partyInvites.length > 0 ? (
          partyInvites.map((invite) => (
            <div key={invite.fromPlayerName} className={styles.invite}>
              ...
              <button
                ...
                onClick={() =>
                  dispatch(removePartyInvite(invite.fromPlayerName))
                }
              >
                Decline
              </button>
