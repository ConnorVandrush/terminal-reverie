...
export const recolorSprite = createAsyncThunk(
  ...
  async (option, { getState }) => {
    ...
    const template =
      ...
      state.gender +
      state.hairStyle +
      state.clothingStyle; // e.g. "grasslandswarriormalehair1style1"
    ...
    const result =
      await window.clientGlobalManager.clientPlayerManager.spriteColorer.recolorSprite(
        img,
        template,
        option,
        color,
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(newSpriteTemplate.fulfilled, (state, action) => {
      ...
      const template =
        ...
        state.clothingStyle; // e.g. "grasslandswarriormalehair1style1"
      const defaultColors =
        window.clientGlobalManager.clientPlayerManager.spriteColorer.defaultColors.get(
          template,
        ) || {};
