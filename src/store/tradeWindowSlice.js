...
const tradeWindowSlice = createSlice({
  ...
  reducers: {
    ...
    serverReceiveTradeRequest: (state, action) => {
      if (
        !state.tradeRequests.some(
          (request) => request.fromPlayerName === action.payload.fromPlayerName,
        )
      ) {
    ...
    clientAcceptTradeRequest: (state, action) => {
      state.tradeRequests = state.tradeRequests.filter(
        (request) => request.fromPlayerName !== action.payload,
      );
      // emit handled in tradeWindowEmitters.js
    },
