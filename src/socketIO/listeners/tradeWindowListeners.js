...
export function createTradeWindowListeners(store)
{
    ...
    window.clientGlobalManager.clientPlayerManager.socket.on('serverTradePartnerAcceptedTrade', (data) =>
    {
        store.dispatch({ type: 'tradeWindow/setErrorMessage', payload: null });
        store.dispatch({ type: 'tradeWindow/setSuccessMessage', payload: data.message });
    });

    window.clientGlobalManager.clientPlayerManager.socket.on('serverTradeError', (data) =>
    {
        store.dispatch({ type: 'tradeWindow/setSuccessMessage', payload: null });
        store.dispatch({ type: 'tradeWindow/setErrorMessage', payload: data.message });
    });

    window.clientGlobalManager.clientPlayerManager.socket.on('serverTradeComplete', (data) =>
    {
        window.clientGlobalManager.clientPlayerManager.updatePlayerCharacterData(data.characterData);
        store.dispatch({ type: 'centerPanel/setCenterPanel', payload: null });
        store.dispatch({ type: 'tradeWindow/setTradePartner', payload: null });
        store.dispatch({ type: 'tradeWindow/setTheirOffer', payload: [] });
        store.dispatch({ type: 'tradeWindow/setYourOffer', payload: [] });
