export default async function tradeWindowEmitters(action, store) {
  ...
  if (action.type === "tradeWindow/clientSendTradeRequest") {
    const response = await socket.emitWithAck("clientSendTradeRequest", {
      toPlayerName: action.payload,
    });
    if (response.success) {
      store.dispatch({
    ...
    const response = await socket.emitWithAck("clientAcceptTradeRequest", {
      fromPlayerName: action.payload,
    });
    if (response.success) {
      store.dispatch({
      ...
      store.dispatch({ type: "tradeWindow/setErrorMessage", payload: null });
      store.dispatch({
        type: "tradeWindow/serverTradeRequestAccepted",
        payload: { fromPlayerName: action.payload },
      });
    } else {
      store.dispatch({
        type: "tradeWindow/setErrorMessage",
    ...
    const response = await socket.emitWithAck("clientTradeDeclined", {
      tradePartner,
    });
    if (response.success) {
    ...
    const tradePartner = store.getState().tradeWindow.tradePartner;
    const response = await socket.emitWithAck("clientOfferGoldInTrade", {
      amt: action.payload,
    ...
    if (response.success) {
      ...
      store.dispatch({
        type: "tradeWindow/setYourOfferGold",
