  ...
  startListeners() {
    this.publicNamespace.on("connection", (socket) => {
      socket.on("clientLogin", async ({ email, password }, cb) => {
        try {
          ...
          if (existingUser.isDead) {
            ...
            this.serverInventoryManager.addItemToInventory(
              existingUser.characterData.inventory,
              1,
              10,
            ); // add 10 herb
            this.serverInventoryManager.addItemToInventory(
          ...
          const mapData = this.serverMapManager.maps.get(
            existingUser.characterData.location.map,
          );
          let playersOnMap = [];
          if (
            this.serverPlayerManager.playersOnMaps.has(
              existingUser.characterData.location.map,
            )
          ) {
            playersOnMap =
              Array.from(
                this.serverPlayerManager.playersOnMaps
                  .get(existingUser.characterData.location.map)
                  ?.entries(),
              ) || [];
          }
          this.serverPlayerManager.socketIdToSocket.set(socket.id, socket);
          this.serverPlayerManager.characterNameToId.set(
            existingUser.characterData.name,
            playerId,
          );
          return cb({
            success: true,
            JWT,
            characterData: existingUser.characterData,
            mapData,
            playersOnMap,
          });
        } catch (error) {
      ...
      socket.on("clientRegister", async ({ email, password }, cb) => {
        try {
          ...
          const newUser = new PlayerAccount({
            email,
    ...
    this.io.on("connection", async (socket) => {
      ...
      socket.on("disconnect", async () => {
        ...
        if (
          this.serverPartyManager.playerParties.has(playerId)
