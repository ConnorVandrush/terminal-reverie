...
class ServerEncounterManager {
  ...
  rollForEncounter(mapName, region, battleback, playerData) {
    ...
    if (this.randInt(1, 100) < regionTable.encounterChance) {
      const entries = Object.values(regionTable).filter(
        (e) => typeof e === "object" && e.weight,
      );
      ...
      for (const entry of entries) {
        ...
        if (random < 0) {
          ...
          if (!playerData.inEncounter) {
            ...
            this.ongoingEncounters.set(
              playerData.characterData.playerId,
              newEncounter,
            );
            return {
              troopData,
              enemyData: troopData.members.map((member) =>
                this.enemies.get(member.enemyId),
              ),
              battleback,
            };
          }
  ...
  endEncounter(playerId) {
    ...
    if (encounter) {
      const playersInEncounter = encounter
        ? encounter.allyList.map((ally) => ally.playerId)
        : [];
    ...
    this.io.on("connection", (socket) => {
      socket.on("clientAllyTurn", (allyTurnData) => {
        ...
        if (playerData.partyData.partyLeaderId > 0) {
          thisEncounter = this.ongoingEncounters.get(
      ...
      socket.on("clientRequestTransferFromEncounter", (cb) => {
        ...
        const mapData =
          this.serverMapManager.maps.get(mapName)?.mapData || null;
        ...
        const playersOnMap =
          this.serverPlayerManager.playersOnMaps.get(mapName) || new Map();
        cb({
          success: true,
          mapData,
          tileset,
