...
class ServerMapManager {
  ...
  canMove(playerData, direction, newLocation, currentLoc) {
    switch (direction) {
      case 2:
        newLocation = {
          ...playerData.characterData.location,
          y: playerData.characterData.location.y + 1,
        };
        break; // down
      case 4:
        newLocation = {
          ...playerData.characterData.location,
          x: playerData.characterData.location.x - 1,
        };
        break; // left
      case 6:
        newLocation = {
    ...
    if (
      !this.isPassable(currentLoc.map, currentLoc.x, currentLoc.y, direction)
    ) {
  ...
  startListeners = () => {
    this.io.on("connection", (socket) => {
      socket.on("clientPlayerMove", async (direction, cb) => {
        ...
        if (
          !playerData ||
          playerData.isTransferring ||
          playerData.preventMovement ||
          ...
          return cb({
            success: false,
            message:
              "Cannot move at this time because already transferring, preventing movement, or in an encounter",
          });
        }
        ...
        if (!result.success) {
          return cb({
            success: false,
            message: "Cannot move at this time because the tile is blocked",
          });
        }
        playerData.characterData.location = result.newLocation;
        const encounter = await this.serverEncounterManager.rollForEncounter(
          currentLoc.map,
          this.getRegion(
            currentLoc.map,
            result.newLocation.x,
            result.newLocation.y,
          ),
      ...
      socket.on("clientPartyMove", async (direction) => {
        const leaderData = this.serverPlayerManager.playersOnline.get(
          socket.playerId,
        );
        if (
          !leaderData ||
          leaderData.isTransferring ||
        ...
        for (const member of partyData.members) {
          const memberData = this.serverPlayerManager.playersOnline.get(
            member.playerId,
          );
          oldPositions.set(member.playerId, {
            ...memberData.characterData.location,
          });
        }
        for (let i = 0; i < partyData.members.length; i++) {
          ...
          if (i === 0) {
            memberData.characterData.location = oldPositions.get(
              leaderData.playerId,
            );
          } else {
            memberData.characterData.location = oldPositions.get(
              previousMember.playerId,
            );
          }
        }
        ...
        const newLocations = partyData.members.map((member) => {
          ...
          return {
            ...
            newLocation: memberData.characterData.location,
          };
        });
        ...
        this.io
          .to(leaderData.characterData.location.map)
          .emit("serverPartyMoved", {
            newLocations,
            inEncounter: leaderData.inEncounter,
          });
        if (encounter) {
          this.io
            .to(`party_${socket.playerId}`)
            .emit("serverPartyEncounter", encounter);
        }
      });
      ...
      socket.on("clientRequestMapTransfer", async (cb) => {
        ...
        const event = this.getEventData(
          playerData.characterData.location.map,
          playerData.characterData.location.x,
          playerData.characterData.location.y,
        );
        ...
        if (event.name.startsWith("Transfer")) {
          ...
          const playersOnMap =
            this.serverPlayerManager.playersOnMaps.get(destinationMap) ||
            new Map();
          ...
          if (this.serverPartyManager.playerParties.has(socket.playerId)) {
            this.io
              ...
              .emit(
                ...
                this.serverPartyManager.playerParties.get(socket.playerId),
              );
          }

          this.serverPlayerManager.playerLeftMap(
            socket,
            socket.playerId,
            playerData.characterData.location.map,
          );
          playerData.characterData.location = { x, y, d, map: destinationMap };
          this.serverPlayerManager.playerJoinMap(
            socket,
            socket.playerId,
            destinationMap,
          );
          ...
          return cb({
            ...
            mapData,
            tileset,
            playersOnMap: Array.from(playersOnMap.entries()),
          });
        }
      });
