class ServerChatManager 
{
    ...
    startListeners = () => 
    {
        this.io.on('connection', (socket) => 
        {
            ...
            {
                const player = this.serverPlayerManager.playersOnline.get(socket.playerId);
                const map = player?.characterData?.location?.map;
                if (player && map) 
                {
                    this.io.to(map).emit('serverBroadcastChatMessage', { message, sender: player.characterData.name });
                }
            });
        });
    }
