...
class ServerInventoryManager {
  ...
  clientBuyItem(playerData, itemId, quantity) {
    ...
    const inventory = characterData.inventory;
    const key = String(itemId);
    const eventData = this.serverMapManager.getEventData(
  ...
  startListeners() {
    this.io.on("connection", (socket) => {
      socket.on(
        "clientUseItem",
        ({ selectedItem, selectedPartyMemberId }, cb) => {
          ...
          const result = this.clientUseItem(
            playerData,
            selectedItem.id,
      ...
      socket.on("clientSendTradeRequest", async ({ toPlayerName }, cb) => {
        ...
        if (toPlayerSocket.id === socket.id) {
          return cb({
            success: false,
            message: `You cannot send a trade request to yourself`,
          });
        }
        cb({ success: true, message: `Trade request sent to ${toPlayerName}` });
        this.io
          .to(toPlayerSocket)
          .emit("serverSendTradeRequest", { fromPlayerName });
      });

      socket.on("clientAcceptTradeRequest", async ({ fromPlayerName }, cb) => {
        if (
          this.serverPartyManager.checkOrthogonalAdjacency(
            socket.playerId,
          ...
          cb({
            ...
            message: `Trade request accepted. Starting trade with ${fromPlayerName}`,
          });
        } else {
          cb({
            success: false,
      ...
      socket.on("clientTradeDeclined", async ({ tradePartner }, cb) => {
        ...
        const partnerSocket = partnerPlayerData
      ...
      socket.on("clientOfferGoldInTrade", async ({ amt, tradePartner }, cb) => {
        ...
        if (amt > playerData.characterData.gold) {
          return cb({
            success: false,
            message: "You do not have that much gold",
          });
        }
        if (partnerSocket) {
      ...
      socket.on(
        ...
        async ({ qty, itemId, itemName, tradePartner }, cb) => {
          const playerData = this.serverPlayerManager.playersOnline.get(
            socket.playerId,
          );
          ...
          const partnerCurrentQty = partnerInvItem
            ? partnerInvItem.quantity
            : 0;

          if (partnerCurrentQty + qty > 99) {
            return cb({
              success: false,
              message: "They cannot hold more than 99 of that item",
            });
          }
        ...
        async (
          ...
          if (partnerCharacterData.tradeOffer == null) {
            if (partnerSocket) {
              this.io
                .to(partnerSocket)
                .emit("serverTradePartnerAcceptedTrade", {
            ...
            this.tradeOffersMatch(
              characterData.tradeOffer,
              partnerCharacterData.tradeOffer,
            )
          ) {
