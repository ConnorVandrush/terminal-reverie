...
class Encounter {
  ...
  checkForWinner() {
    if (this.deadAllies.length === this.allyList.length) {
      ...
      this.serverEncounterManager.serverPartyManager.playerParties.delete(
        this.allyList[0].playerId,
      );
    } else if (this.deadEnemies.length === this.enemyList.length) {
      const rewards = this.enemyList.reduce(
        (acc, enemy) => {
      ...
      rewards.experience = Math.floor(
        rewards.experience / this.allyList.length,
      );
      this.allyList.forEach(async (ally) => {
      ...
      return {
        rewardMessage: `You are victorious! You gained ${rewards.gold} gold and ${rewards.experience} experience!`,
        gold: rewards.gold,
  ...
  processTurn(allyTurnData) {
    ...
    if (
      this.allyTurns.length ===
      this.allyList.length - this.deadAllies.length
    ) {
      this.setupNextTurn();
      for (const allyAction of this.allyTurns) {
        if (!this.isAbleToAct(allyAction.characterData)) continue;
        const allyTurnResult = CharacterData.processTurn(
          allyAction.characterData,
          allyAction,
          this.enemyList,
        );
      ...
      if (rewards) {
        this.serverEncounterManager.broadcastTurnResults(
          this.room,
          this.allyTurnResults,
          this.enemyTurnResults,
          this.deadAllies,
